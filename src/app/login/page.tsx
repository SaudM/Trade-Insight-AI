      ...
      <div className="relative z-10 w-full" style={{ maxWidth: 400 }}>
        ...
        <div style={{ background: PT.bg, border: `1px solid ${PT.border}`, borderRadius: 16, padding: '32px 28px' }}>
          ...
          <div className="mb-7">
            ...
            <p style={{ fontSize: 14, color: PT.muted }}>登录您的复盘 · AI量化账户</p>
          </div>

          {/* Form */}
          {firebase ? (
            <Suspense fallback={
              <div style={{ textAlign: 'center', fontSize: 14, color: PT.muted, padding: '24px 0' }}>加载中…</div>
            }>
              <LoginForm />
            </Suspense>
          ) : (
            <div style={{ textAlign: 'center', fontSize: 14, color: PT.muted }}>
              Firebase 未配置，请检查环境变量。
            </div>
          )}
        </div>
